...
# 设置保存文件
ipaddress=$(ip address | ag -o '(?<=inet )\d+\.\d+\.\d+\.\d+(?=\/2)')
...
base_check() {
  ...
  if [ $OS = 'Centos' ]; then
    rpm -Va | grep -E "/etc|/usr/bin|/sbin|/usr/local/bin" | tee -a "$filename"
  else
...
env_check() {
  ...
  echo "${PROMPT_COMMAND}" | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
  #LD_LIBRARY_PATH
...
miner_check() {
  echo -e "######挖矿木马检查######\n" | tee -a "$filename"
  echo -e "\e[00;31m[+]挖矿进程检测\e[00m" | tee -a "$filename"
  ...
  find / ! -path "/proc/*" ! -path "/sys/*" ! -path "/run/*" ! -path "/boot/*" -regextype posix-extended -regex '.*systemctI|.*kworkerds|.*init10.cfg|.*wl.conf|.*crond64|.*watchbog|.*sustse|.*donate|.*proxkekman|.*cryptonight|.*sustes|.*xmrig|.*xmr-stak|.*suppoie|.*ririg|gg1.conf|.*cpuminer|.*xmr|.*xig|.*ddgs|.*minerd|.*hashvault|\.kthreadd|.*httpdz|.*kerbero|.*2t3ik|.*qW3xt|.*ztctb' -type f | tee -a "$filename"
  echo -e "\n" | tee -a "$filename"
}
